...
export default function LearningPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Calculate stats
        const resolvedCount = learningData.filter(l => l.outcome === 'RESOLVED').length;
        const avgImprovement = learningData.length > 0 
          ? learningData.reduce((sum, l) => sum + (l.confidence_after - l.confidence_before), 0) / learningData.length 
          : 0;
        
        const bestPerforming = statsData?.learning_stats?.length > 0 
  ...
  const columns: DataTableColumn<LearningHistory>[] = [
    ...
    },
    {
      ...
      header: 'Outcome',
      render: (value) => {
        const variant = value === 'RESOLVED' ? 'success' : 
        ...
        >
          <div className="space-y-4">
            {improvementStats?.learning_stats?.slice(0, 5).map((stat: any) => (
              <div key={`${stat.issue_type}-${stat.action_type}`} className="p-4 bg-white border border-slate-200 rounded-xl">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-semibold text-slate-900">{stat.issue_type}</h4>
                  <span className="text-sm text-slate-500">{stat.total_records} records</span>
                </div>
                <p className="text-sm text-slate-600 mb-3">{stat.action_type}</p>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-slate-600">Success Rate:</span>
                    <p className="font-semibold text-green-600">{Math.round(stat.success_rate)}%</p>
                  </div>
                  <div>
                    <span className="text-slate-600">Avg Improvement:</span>
                    <p className="font-semibold text-blue-600">+{Math.round(stat.avg_improvement * 100)}%</p>
                  </div>
                </div>
              </div>
            )) || (
        ...
        >
          <div className="space-y-4">
            {['RESOLVED', 'FAILED', 'PARTIAL'].map(outcome => {
              const count = learningRecords.filter(l => l.outcome === outcome).length;
              const percentage = learningRecords.length > 0 ? (count / learningRecords.length) * 100 : 0;
              
              return (
                <div key={outcome} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                  <div className="flex items-center space-x-3">
                    <StatusBadge 
                      status={outcome} 
                      variant={outcome === 'RESOLVED' ? 'success' : 
                    ...
                    <div className="w-20 bg-slate-200 rounded-full h-2">
                      <div 
                        className={`h-2 rounded-full ${
                          ...
                          outcome === 'FAILED' ? 'bg-red-500' : 'bg-yellow-500'
                        }`}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl">
            <h4 className="font-semibold text-slate-900 mb-2">Confidence Growth</h4>
          ...
          <div className="p-6 bg-gradient-to-br from-purple-50 to-violet-50 rounded-xl">
            <h4 className="font-semibold text-slate-900 mb-2">Learning Velocity</h4>
            <p className="text-sm text-slate-600 mb-4">
