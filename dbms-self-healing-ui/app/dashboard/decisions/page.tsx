...
export default function DecisionsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Calculate stats
        const autoHealCount = decisionData.filter(d => d.decision_type === 'AUTO_HEAL').length;
        const adminReviewCount = decisionData.filter(d => d.decision_type === 'ADMIN_REVIEW').length;
        const avgConf = decisionData.length > 0 
          ? decisionData.reduce((sum, d) => sum + d.confidence_at_decision, 0) / decisionData.length 
          : 0;
  ...
  const columns: DataTableColumn<DecisionLog>[] = [
    ...
    },
    {
      ...
      render: (value) => {
        const variant = value === 'AUTO_HEAL' ? 'success' : 
                       value === 'ADMIN_REVIEW' ? 'warning' : 'default';
        ...
        >
          <div className="space-y-4">
            {['AUTO_HEAL', 'ADMIN_REVIEW', 'ESCALATED'].map(decisionType => {
              const count = decisions.filter(d => d.decision_type === decisionType).length;
              const percentage = decisions.length > 0 ? (count / decisions.length) * 100 : 0;
              
              return (
                <div key={decisionType} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                  <div className="flex items-center space-x-3">
                    <StatusBadge 
                      status={decisionType} 
                      variant={decisionType === 'AUTO_HEAL' ? 'success' : 
                              decisionType === 'ADMIN_REVIEW' ? 'warning' : 'default'} 
                    />
                    ...
                    <div className="w-20 bg-slate-200 rounded-full h-2">
                      <div 
                        className={`h-2 rounded-full ${
                          decisionType === 'AUTO_HEAL' ? 'bg-green-500' :
                          decisionType === 'ADMIN_REVIEW' ? 'bg-yellow-500' : 'bg-slate-500'
                        }`}
        ...
        >
          <div className="space-y-4">
            {decisions.slice(0, 5).map((decision) => (
              <div key={decision.decision_id} className="p-4 bg-white border border-slate-200 rounded-xl">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    <span className="font-mono text-sm text-slate-500">#{decision.decision_id}</span>
                    <StatusBadge 
                      status={decision.decision_type} 
                      variant={decision.decision_type === 'AUTO_HEAL' ? 'success' : 'warning'} 
                    />
                  </div>
                ...
                <p className="text-sm text-slate-700 mb-2">{decision.decision_reason}</p>
                <div className="flex items-center space-x-2">
                  <span className="text-xs text-slate-600">Confidence:</span>
                  <div className="w-16 bg-slate-200 rounded-full h-1.5">
                    <div 
                      className="bg-blue-500 h-1.5 rounded-full" 
                      style={{ width: `${decision.confidence_at_decision * 100}%` }}
                    />
                  </div>
                  <span className="text-xs font-medium">{Math.round(decision.confidence_at_decision * 100)}%</span>
                </div>
              </div>
            ))}
          </div>
